...
{
    public partial class form_2sp_exponencial : Form
    {
        bool arribo;
        int c;
            ...
            if (msj == 0)
            {
                ...
                for (int i = 0; i < tiempos.Length; i++)
                {
                    ...
                    if (arribo)
                    {
                        TDS.Add(tiempos[i]);
                        TDS.Add(tiempos[i]);
                        MultiplesServidores = new ServidoresParalelo(c, Convert.ToDouble(this.txtTEA.Text), TDS);
                    }
                    else
                    {
                        TDS.Add(Convert.ToDouble(this.txtTDS1.Text));
                        TDS.Add(Convert.ToDouble(this.txtTDS2.Text));
                        MultiplesServidores = new ServidoresParalelo(c, tiempos[i], TDS);
                    }
        ...
        private int Validar(Control.ControlCollection controls)
        {
            ...
            if (!est)
            {
                ...
                else if (this.chkServicio.Checked)
                {
                    ...
                    if (!(Double.TryParse(txtTDS1.Text, out res)) || !(Double.TryParse(txtTDS2.Text, out res)))
                    {
                        msj = 3;
                    }
                    else if ((Convert.ToDouble(txtTDS1.Text) < 0.1 || Convert.ToDouble(txtTDS1.Text) > 1) || (Convert.ToDouble(txtTDS2.Text) < 0.1 || Convert.ToDouble(txtTDS2.Text) > 1))
                    {
                        msj = 5;
                    }
                }
